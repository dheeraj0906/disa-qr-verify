...
export default function AdminReviewScreen() {
  ...
  async function loadLog() {
    try {
      const res = await taskLogsApi.list();
  ...
  async function handleReject() {
    if (!remark.trim()) {
      Alert.alert('Remark Required', 'Please enter a reason for rejection.');
      return;
    }
    setSubmitting(true);
    try {
    ...
    <View style={{ flex: 1, backgroundColor: '#F5F5F5' }}>
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        ...
        <Text style={{ color: '#666', fontSize: 14, marginBottom: 16 }}>
          {log.worker_name} • {formatIST(log.scanned_at)}
        </Text>
        ...
        {(log.before_photo_url || log.after_photo_url) ? (
          <View style={{ gap: 12, marginBottom: 20 }}>
            {log.before_photo_url && (
              <View>
                <Text style={{ color: '#666', marginBottom: 4, fontWeight: '500' }}>BEFORE</Text>
                <Image source={{ uri: log.before_photo_url }} style={{ width: '100%', height: 240, borderRadius: 12 }} resizeMode="cover" />
              </View>
            )}
            {log.after_photo_url && (
              <View>
                <Text style={{ color: '#666', marginBottom: 4, fontWeight: '500' }}>AFTER</Text>
                <Image source={{ uri: log.after_photo_url }} style={{ width: '100%', height: 240, borderRadius: 12 }} resizeMode="cover" />
              </View>
            )}
          </View>
        ) : (
          <View style={{ padding: 32, alignItems: 'center', backgroundColor: '#fff', borderRadius: 12, marginBottom: 20 }}>
            <Text style={{ color: '#666' }}>No photos attached to this submission.</Text>
          </View>
        )}
        ...
        {log.duration && (
          <View style={{ backgroundColor: '#fff', borderRadius: 12, padding: 14, marginBottom: 16 }}>
            <Text style={{ color: '#666', fontSize: 13 }}>Time taken: <Text style={{ fontWeight: 'bold', color: '#000' }}>{log.duration}</Text></Text>
          </View>
        )}
        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }}>
          <View style={{ backgroundColor: '#fff', borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 24 }}>
            ...
            <View style={{ flexDirection: 'row', gap: 12, marginTop: 16 }}>
              ...
              >
                {submitting ? <ActivityIndicator color="#fff" /> : <Text style={{ fontWeight: 'bold', color: '#fff' }}>Submit Rejection</Text>}
              </TouchableOpacity>
            </View>
          </View>
        </View>
